...
public class BandHelper {
	...
	public List<Musician> viewBandMembers(Band bandToView) {
		
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<Musician> typedQuery = em.createQuery("select m from Musician m where m.band = :selectedBandName", Musician.class);
		typedQuery.setParameter("selectedBandName", bandToView);
		List<Musician> foundMembers = typedQuery.getResultList();
		em.close();
		System.out.println(foundMembers.toString());
		return foundMembers;
	}
