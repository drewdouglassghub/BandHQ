...
public class BandNavServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if (act.equals("view members")) {
			try {
				Integer tempId = Integer.parseInt(request.getParameter("id"));
				Band bandToView = dao.searchForBandById(tempId);
				
				List<Musician> bandMembers = dao.viewBandMembers(bandToView);
				request.setAttribute("bandMembers", bandMembers	);
				request.setAttribute("bandName", bandToView);
				getServletContext().getRequestDispatcher("/MemberListByBand.jsp").forward(request, response);		
				}
				catch (NumberFormatException e) {
					System.out.println("Forgot to click a button.");
				} 
